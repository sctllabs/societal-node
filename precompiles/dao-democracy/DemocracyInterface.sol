...
/// @author Societal Labs
/// @title Pallet DAO Democracy Interface
...
interface Democracy {
    ...
    function publicPropCount(uint32 dao_id) external view returns (uint256);
    ...
    function depositOf(uint32 dao_id, uint256 propIndex) external view returns (uint256);
    ...
    function lowestUnbaked(uint32 dao_id) external view returns (uint256);
    ...
    function ongoingReferendumInfo(uint32 dao_id, uint32 refIndex)
        external
        view
        returns (
            uint256,
    ...
    function finishedReferendumInfo(uint32 dao_id, uint32 refIndex)
        external
        view
    ...
    function propose(uint32 dao_id, bytes memory proposal, uint256 value) external;
    ...
    function propose_with_meta(uint32 dao_id, bytes memory proposal, uint256 value, bytes memory meta) external;
    ...
    function second(uint32 dao_id, uint256 propIndex, uint256 secondsUpperBound) external;
    ...
    /// @param dao_id DAO ID
    /// @param refIndex The index of the referendum you are interested in
    function removeVote(uint32 dao_id, uint256 refIndex) external;
    ...
    function unDelegate(uint32 dao_id) external;
    ...
    /// @param dao_id DAO ID
    function unlock(uint32 dao_id, address target) external;
    ...
    event Proposed(uint32 dao_id, uint32 indexed proposalIndex, uint256 deposit);
    ...
    event Seconded(uint32 dao_id, uint32 indexed proposalIndex, address seconder);
    ...
    event Delegated(uint32 dao_id, address indexed who, address target);

    /// @dev An account undelegated.
    /// @custom:selector 42176493fdfcada70cc1bcf321c9a2314e9571a9fe53c54a5385a1eeac8bc1d7
    /// @param dao_id DAO ID
    /// @param who address Address of the delegator.
    event Undelegated(uint32 dao_id, address indexed who);
}
