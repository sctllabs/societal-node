...
use dao_primitives::{
	...
	DaoPalletSubscriptionDetailsV1, DaoSubscription as Subscription, DaoSubscriptionDetailsV1,
	DaoTxValidityError, DemocracySubscriptionDetailsV1, MembershipSubscriptionDetailsV1,
	TreasurySubscriptionDetailsV1, VersionedDaoSubscriptionDetails, VersionedDaoSubscriptionTier,
};
			...
			DaoSubscription::ensure_active(*dao_id, |subscription| {
				...
				match asset_accounts_total {
					...
					Some(accounts) => match subscription.clone().details {
						VersionedDaoSubscriptionDetails::Default(details) => {
							let DaoSubscriptionDetailsV1 { max_members, .. } = details;
							ensure!(
...
fn validate_proposal(
	...
	details: VersionedDaoSubscriptionDetails<BlockNumber, Balance>,
...
fn validate_dao_pallet(
	...
	match details {
		VersionedDaoSubscriptionDetails::Default(details) => {
			let DaoSubscriptionDetailsV1 { dao, .. } = details;
...
fn validate_bounties(
	...
	match details {
		VersionedDaoSubscriptionDetails::Default(details) => {
			let DaoSubscriptionDetailsV1 { bounties, .. } = details;
...
fn validate_council(
	...
	match details.clone() {
		VersionedDaoSubscriptionDetails::Default(subscription_details) => {
			let DaoSubscriptionDetailsV1 { council, .. } = subscription_details;
...
fn validate_democracy(
	...
	match details.clone() {
		VersionedDaoSubscriptionDetails::Default(subscription_details) => {
			let DaoSubscriptionDetailsV1 { democracy, .. } = subscription_details;
			if let RuntimeCall::DaoDemocracy(..) = call {
				...
				match call {
					...
					RuntimeCall::DaoDemocracy(pallet_dao_democracy::Call::propose_with_meta {
						...
						let (proposal_dispatch_data, _) =
							DaoDemocracy::extract_original_call(proposal).map_err(|_| {
								InvalidTransaction::Custom(DaoTxValidityError::Unknown.into())
							})?;
...
fn validate_tech_committee(
	...
	match details.clone() {
		VersionedDaoSubscriptionDetails::Default(subscription_details) => {
			let DaoSubscriptionDetailsV1 { tech_committee, .. } = subscription_details;
...
fn validate_council_membership(
	...
	match details {
		VersionedDaoSubscriptionDetails::Default(details) => {
			let DaoSubscriptionDetailsV1 { council_membership, .. } = details;
			if let RuntimeCall::DaoCouncilMembers(..) = call {
				let MembershipSubscriptionDetailsV1 {
					add_member,
					remove_member,
					swap_member,
					change_key,
...
fn validate_tech_committee_membership(
	...
	match details {
		VersionedDaoSubscriptionDetails::Default(details) => {
			let DaoSubscriptionDetailsV1 { tech_committee_membership, .. } = details;
...
fn validate_treasury(
	...
	match details {
		VersionedDaoSubscriptionDetails::Default(details) => {
			let DaoSubscriptionDetailsV1 { treasury, .. } = details;
			if let RuntimeCall::DaoTreasury(..) = call {
				let TreasurySubscriptionDetailsV1 { spend, transfer_token } =
					treasury.ok_or(error)?;
